...
module.exports = function (sails) {
  ...
  return {
    /**
     * Initialize this project's Gulpfile tasks
     ...
     * @param {Function} [cb_afterTaskStarted] - fires when the Grunt task has been started (non-production) or finished (production)
     */
      ...
      child.stdout.on('data', function (consoleMsg) {
        ...
        if (consoleMsg.match(/Fatal error/g)) {
          ...
          if (consoleMsg.match(/Unable to find Gulpfile/i)) {
            sails.log.info('Gulpfile could not be found.');
